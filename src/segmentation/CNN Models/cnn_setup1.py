...
MODEL_PATH = os.path.join(BASE_PATH, 'bubble_segmentation_model2.h5')
...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        X = np.zeros((len(indexes), *self.target_size, 3), dtype=np.float32)
        y = np.zeros((len(indexes), *self.target_size, 1), dtype=np.float32)

        for i, idx in enumerate(indexes):
            try:
                img = cv2.imread(self.image_paths[idx])
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) if img is not None else np.zeros((*self.target_size, 3), dtype=np.uint8)
                img = cv2.resize(img, (self.target_size[1], self.target_size[0])) / 255.0
                ...
                X[i], y[i] = np.zeros_like(X[i]), np.zeros_like(y[i])
...
def create_unet_model(input_size=(416, 576, 3)):
    ...
    model.compile(optimizer=Adam(learning_rate=1e-4),
                  ...
                  metrics=[iou_metric,dice_coefficient, precision_metric, recall_metric, pixel_accuracy])
